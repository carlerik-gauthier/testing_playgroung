...
class ClassifierDataTest:
    ...
    @pytest.fixture(scope='function')
    def get_rf_classification_model(self):
        # input_df = pd.DataFrame(data={'age': [3.5, 22, 3.5, 30, 4],
        #                              'Pclass': [1, 2, 3, 1, 2],
        #                              'women_child': [2, 2, 2, 0, 2],
        #                              'female': [13, 13, 0, 0, 0],
        #                              'survived': [1, 0, 0, 1, 0]})
        ...
        checks = [(self._input_df, self.features, self.target_col, 2),
                  (self._input_df, ['age', 'Pclass', 'women_child', 'female'], 'survived', 5)]
        return checks
